...
class PPOTrainer:
    ...
    def __init__(self, agents: Dict[str, Agent], env: MultiAgentEnv, config: Dict[str, Any]):
        ...
        if self.config['load_weights_from_step'] > 0:
            for agent_id, agent in self.agents.items():
                agent.model = torch.load(os.path.join(Path.home() / "tb_logs" / f"{self.config['load_weights_from_prefix']}", f"{agent_id}_{self.config['load_weights_from_step']}.pt"),
    ...
    def train(self, num_iterations: int,
        ...
        for step in trange(starting_iteration, starting_iteration + num_iterations, disable=disable_tqdm):
            timer.checkpoint()
            step += self.config['load_weights_from_step']
